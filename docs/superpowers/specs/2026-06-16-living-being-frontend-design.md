...
4. **The spinal cord is the being's own body — and its conductor.** The spine + vertebrae are **integral anatomy of the being, not a tool that appears.** The brain and its spinal cord are **one continuous body, present in every state** — exactly as a real brain and spinal cord are a single nervous system. At rest it reads as the **brainstem** the user speaks into (coiled/short). As work begins, that same cord **extends and reveals its vertebrae**, and **each vertebra holds a tab.** The being never sprouts a separate "UI spine" — the spine *is* the being. One organ, one body, rest → conduction.
   ...
   **Anatomical fidelity (operator directive + reference `refs/IMG_2852-human-nervous-system.jpeg`):** the brain is a real human brain, so the spinal cord, **vertebral segmentation, and bilateral nerve roots are human-anatomy-referenced too** — a real CNS silhouette (cord descending from the brainstem; paired nerve roots branching at each segment; fanning progressively longer toward the base into a cauda-equina spray), rendered in the being's luminous palette — **faithful in STRUCTURE, ethereal in MATERIAL** (never a grotesque cadaver). Each **vertebral segment is an addressable anchor that SEATS a tab in orchestration** (this is the same backbone as State 5's conductor — anatomy now, conductor later).
