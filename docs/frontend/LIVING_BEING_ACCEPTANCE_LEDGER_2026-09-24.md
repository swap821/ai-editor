...
| PERF-01 | Desktop frame and input budget (4) | Named desktop/GPU/browser, complete composer frame accounting, p50/p95 and input latency against budgets frozen before visual-load changes. | 2026-09-24: counter code now aggregates Three.js draw calls after EffectComposer and preserves missing GPU counters as unavailable; focused multi-pass helper tests pass. No browser capture, named desktop, p50/p95/input distribution or GPU-time evidence. `automated-verified` partial; **0**. |
