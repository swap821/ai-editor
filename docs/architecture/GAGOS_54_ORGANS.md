...
| 30 | Communication and Human-State Interpreter | `HumanStateInterpreterAuthority` | 28 | **Tier 3 update:** `classify_human_state()` is now genuinely wired into the real, live `/api/v1/chat` turn path -- `stream_conversation()` classifies every non-empty user turn, emits a new advisory `human_state` SSE frame (always visible, `user_correctable=True`, `grants_authority=False`) before the route/text frames, and best-effort-persists it via a new `HumanStateHypothesisStore` (append-only, digest-verified, migration 0010). Still missing: the classifier has not been measured against real production operator traffic -- only synthetic labeled examples exist today; a genuine gap that closes naturally as real chat traffic accumulates in the new store. |
| 31 | Human Representative Context Compiler | `RepresentativeContextCompilerAuthority` | 29 | **Tier 4 update:** `route_intelligence_request()` genuinely calls `compile_representative_context()` on every Council reasoning call today, but `gateway_reasoning.py`'s `GatewayRoutedCouncilLLMClient.complete()` (the sole caller) computed the `context_digest` and then silently discarded it -- its own docstring's "genuine audit-able context_digest" claim was not actually true. Now it is: exposed as a real `last_context_digest` attribute and logged unconditionally (`council_gateway_context_compiled`). Still missing: the OLDER `IntelligenceRequest`/`ModelCallRequest` dataclasses (`aios/runtime/intelligence_gateway.py`, `aios/domain/privacy/contracts.py`) genuinely have no `context_digest` field, and adding one meaningfully would require wiring `compile_representative_context()` into `IntelligenceGateway`/`IntelligenceHiringService` -- the exact multi-system reconciliation the operator deliberately scoped out of organ 32 given the regression risk to real, currently-working worker plan/repair reasoning and the hiring flow. |
| 32 | Universal Intelligence Gateway | `UniversalIntelligenceGatewayAuthority` | 41 | **Tier 4 update (operator-confirmed scope: streaming variant only):** grounding found organ 32 is not just "add streaming" -- three separate, independent gateway-shaped systems exist (the Slice-30 `route_intelligence_request()`; the older `aios.runtime.intelligence_gateway.IntelligenceGateway`, confirmed via grep to be genuinely load-bearing for real worker plan/repair reasoning in `aios/runtime/worker_api.py`; and `aios.application.models.hiring_service.IntelligenceHiringService`). Operator chose streaming-variant-only over full reconciliation, given the real regression risk to two currently-working production paths. New `stream_intelligence_request()` (`aios/application/intelligence/gateway.py`) is the streaming counterpart to `route_intelligence_request()` for text-chunk model calls: same upfront identity/constitution validation, emergency-stop check, and context compilation (shared via a new `_validate_and_compile()` helper so the two entrances can never silently drift), but yields chunks as they arrive with independent per-chunk redaction rather than buffering the whole reply -- an honest, documented limitation (cannot catch a secret split across a chunk boundary) that is still a strict improvement over `aios/api/main.py::_stream_chat_chunks()`'s current zero redaction. Still missing: wiring chat (`/api/v1/chat`) and the agentic forge (`/api/generate`) through it. Chat only has a `session_id`, not an authenticated `Principal`/operator identity today (an intentional, existing design so anonymous local chat keeps working) -- wiring it would need the same "real operator -> gate; no operator -> stay off" conditional `gateway_reasoning.py` already established, a real change to the single most heavily used endpoint in the system, deliberately not risked in the same pass as building the capability itself. The forge's `ToolAgent.run()` also yields structured `dict` tool-call events, a genuinely different shape this variant does not cover. The 2 other competing gateway implementations remain unreconciled, per the operator's own scope decision. |
...
| 42 | Recovery and Resumption | `RecoveryResumptionAuthority` | 41 | **Tier 4 update:** `MissionTransitionJournal` wiring is now real for the Council pipeline, its primary and most heavily used execution path -- `CouncilOrchestrator` appends all 11 real states at their genuine points (`aios/council/council_orchestrator.py`, `aios/api/routes/council.py`), proven end to end with a real mission run through `deliberate() -> approve() -> execute() -> PROMOTED` asserting the exact ordered history (`tests/test_council_orchestrator.py`). Still missing: the Maintenance repair pipeline. Grounded precisely (Tier 3 continuation) before attempting the same wiring: `MISSION_TRANSITION_ORDER` requires `APPROVED` strictly before `WORKSPACE_CREATED`, but no production HTTP route can move a maintenance-created mission to `MissionState.APPROVED` today -- `council_approve()`, the only production caller of `MissionService.approve()`, constructs its own `MissionService` against `config.COUNCIL_RUNTIME_DIR/missions.db`, a genuinely different SQLite file from the `config.MISSION_STATE_DB` `MaintenanceConvergenceService` actually uses; a maintenance mission_id there raises `MissionNotFoundError` and silently falls into a legacy no-op fallback. `tests/test_maintenance_api.py`'s own happy path confirms this indirectly, reaching `APPROVED` by calling `.approve()` in-process, bypassing HTTP entirely. Closing this organ's Maintenance half needs a real maintenance-approval HTTP route built first -- unplanned, separate scope beyond journal wiring, not attempted here. |
...
| 45 | Constitutional Amendment Authority | `ConstitutionalAmendmentAuthority` | 37 | **Tier 3 update:** the HTTP surface now exists for 6 of the 7 transitions -- new `ActionType.CONSTITUTIONAL_AMENDMENT_*` members plus real `RouteAuthority` entries in `aios/policy/kernel.py` for `POST /api/v1/governance/amendments/propose` (+ `{id}/critique`, `/simulate`, `/reject`, `/ratify`, `/activate`) and `GET .../{id}` + `.../{id}/history`. Ratify/activate are YELLOW-tier with `confirm_required=True`, reusing the exact same generic two-phase capability issue/consume mechanism every other YELLOW route already uses -- no new capability-issuance semantics invented. Client-supplied `proposed_by`/`proposer_type` are never trusted, both are derived from the real authenticated principal. Proven with 10 real HTTP tests including a genuine foundation-law refusal and a real ratify -> activate chain producing a real next constitution digest. Still missing: no `/rollback` route -- `rollback_amendment()` needs a durable history of `ConstitutionSnapshotV1` objects to find "the exact predecessor" of an activation, and no such store exists (`build_constitution_snapshot()`'s own docstring already documents this, tied directly to organ 25's own "durable cross-restart persistence... remains genuinely unbuilt" blocker); a routing-table entry with no real endpoint would be misleading, so none was added. |
| 46 | Constitutional Learning Organ | `ConstitutionalLearningAuthority` | 38 | **Tier 3 update:** the HTTP surface now exists, reusing organ 45's just-established pattern exactly -- `POST .../lessons/propose`, `GET .../lessons/{id}` + `/history`, `POST .../lessons/{id}/draft-amendment` (calls the real `lesson_to_amendment_proposal()`, persists both the updated lesson and the resulting real `ConstitutionalAmendmentProposalV1`, which is then independently fetchable and ratifiable through organ 45's own routes -- proven end to end in tests), and `POST .../lessons/check-simulations` (a pure, stateless gate over `require_all_simulations_pass()`). 8 new HTTP tests, including a genuine authority-reduction refusal at draft-amendment time. Organ 45's capability-issuance-table gap this organ's blocker previously named is now resolved. Still missing (unchanged scope): the 9 named adversarial simulations remain a fixed catalog this module checks results FOR, not simulations it runs -- a genuine, not-fabricatable gap. |
| 52 | Observability and Health Organ | `ObservabilityAuthority` | 40 | **Tier 4 update (2 of 3 pieces closed):** `aios/api/main.py`'s HTTP middleware wiring (previous update) plus `aios/council/queen_service.py`'s `QueenService._drain_loop()` -- a persistent `asyncio.create_task()` background queue drainer started once at service startup, outliving any single caller, so it does not inherit a request's trace context for free. Now binds a fresh `TraceContext` derived from each dequeued item's own `mission_id` immediately before handling it, proven with a real test asserting `get_trace_context()` inside the handler carries that exact mission id. Investigated `aios/application/workers/scheduler.py`'s `WorkerScheduler` too and confirmed it does **not** need the same fix: unlike the drain loop, `_pump()` calls `asyncio.create_task()` freshly for every admitted ticket (not once at startup), and Python's own `asyncio.Task` semantics copy the current contextvars context at creation time -- verified empirically with a standalone script, not assumed -- so a ticket submitted from within a request's own task already carries that trace context for free, the same mechanism `BackgroundTasks` relies on. Still missing: the Docker executor container is a genuinely separate OS process (`DockerRunner` submits jobs via `subprocess.Popen`, not HTTP -- confirmed by reading it directly), entirely outside Python's `contextvars`. Propagating a trace id across that boundary means passing it as an env var into the subprocess AND having `aios.executor_service`'s own entry point (a separate, sandboxed process) read and rebind it -- real cross-process work touching security-sensitive isolated code, deliberately not attempted under time pressure. |
