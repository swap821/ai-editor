...
| 41 | Promotion, Checkpoint and Rollback (live proof) | `PromotionRollbackLiveAuthority` | 40 | **Tier 4 update:** checkpoint/restore is local filesystem snapshotting (HMAC-signed manifest, no Docker anywhere -- confirmed by reading `checkpoint.py`/`runtime.py` in full), and `aios/api/deps.py`'s `get_checkpoint_creator()`/`get_checkpoint_restorer()` are the real adapters already wired into the real production route `POST /api/v1/maintenance/repairs/run` -- `test_maintenance_api.py`'s existing end-to-end test already exercises the happy path through them. What was missing was a test of the *failure* branch with the real (non-lambda-stub) adapters. New `test_apply_or_smoke_failure_restores_exact_bytes_via_real_checkpoint_authority` (`tests/test_promotion_authority.py`) calls `promote()` with the real `get_checkpoint_creator()`/`get_checkpoint_restorer()`, forces a smoke-test failure after a real diff is applied, and asserts the project file's bytes are restored to their exact pre-promotion value on disk -- closing "never touching...the real CheckpointAuthority/WorkspacePromotionRuntime persistence path." Still missing: an authoritative post-promotion receipt. `PostPromotionVerificationReceipt` exists as a designed schema with zero producers -- building one needs a `verifier_id`/`verifier_version` identity with no existing analog for the promotion step and a new post-apply project-digest concept nothing computes today; `PromotionResult` itself has no field to carry a receipt on. A genuine, separate design task, not attempted here. |
...
| 52 | Observability and Health Organ | `ObservabilityAuthority` | 40 | **Tier 4 update:** `aios/api/main.py`'s existing `bind_request_context` HTTP middleware now also builds a real `TraceContext` from request headers (reusing the already-computed `request_id`) and binds it via `bind_trace_context()` for the request's duration. Proven empirically, not assumed: a new test shows a synchronous in-request call sees the bound context; a second new test shows a function scheduled via FastAPI's `BackgroundTasks` sees it too (Starlette awaits an async background callable in the same task; anyio's `run_in_threadpool` copies context into the worker thread for a sync one) -- so every model call, clerk job, verification, promotion, rollback, or emergency-stop check reached synchronously (including via `BackgroundTasks`) from an HTTP request now carries a real trace context with zero per-call-site wiring. Still missing: anything outside a request's own task gets nothing for free -- `aios/council/queen_service.py`'s persistent `_drain_loop()` (started once via `asyncio.create_task()`, outliving any single request) and `aios/application/workers/scheduler.py`'s background scheduling loop both need a trace id carried on the queued item itself and rebound at dequeue time; the Docker executor container is a genuinely separate process, needing cross-process propagation (env var or job-payload field). Neither attempted here. |
