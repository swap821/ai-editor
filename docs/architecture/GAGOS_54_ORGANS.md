...
- **Source ledger sha256:** `edc8afd34c61663d80621c853abeec1dfc31b3be1effadae9e67d98206c9a0d2`
...
| # | Organ | Authority owner | Residual |
|---|-------|------------------|----------|
| 55 | Governance Conformance Evaluation (Refusal Reel) | `GovernanceConformanceAuthority` | ALL FIVE MISSIONS BLOCKED (0/5). Mapped against the real event vocabulary on 2026-09-03; adjudicators are correct, the signals do not exist. M1: no production record names WHICH control refused -- 'scope_lock', 'red_zone', 'execute_approved' have ZERO non-test emissions under aios/ and 'security_gateway' has one whose only caller is a test; the persuasion text is never recorded. M2: nothing durably pairs a minted verification strength with the evidence it rests on outside Council Runtime. M3: 'injection_detected' has ZERO occurrences anywhere -- no injection is recorded as an event for either source, and tool output is never classified at all. M4: CapabilityAuthority.revoke() has no production caller, no RUNNING->KILLED transition exists, and AUTONOMY_GRANT_CHANGED has zero publishers. M5: write ops never compile into skills (cerebellum.py:41). Every fix is non-frozen.<br>ARCHITECTURAL: aios/runtime/cortex_bus.py:106 declares _AUTHORITY_EVENT_PREFIXES = ('skill.','autonomy.','approval.','verdict.','zone.','grant.') and refuses to persist any of them (ADR 4.1, 'decisions stay synchronous on the verifier's return value'). Those are precisely the event families this organ must assert over, so governance decisions leave no durable structured trace by design. Closing this is a design decision for the operator, not a bug fix -- it trades an explicit architectural choice for observability.<br>Outside-machine: no live evidence yet, and none is possible until the above is closed -- there is nothing to observe. requires_live_evidence is true and honestly so. Like organ 44 this cannot carry a release/phase4 artifact, so its evidence will be operator-attested when it exists.<br>No integration test yet. The adjudicators are proven as units (28 tests); nothing drives them end-to-end because no observation can currently be built. |
