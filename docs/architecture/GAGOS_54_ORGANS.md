...
| 42 | Recovery and Resumption | `RecoveryResumptionAuthority` | 41 | **Tier 4 update:** `MissionTransitionJournal` wiring is now real for the Council pipeline, its primary and most heavily used execution path -- `CouncilOrchestrator` appends all 11 real states at their genuine points (`aios/council/council_orchestrator.py`, `aios/api/routes/council.py`), proven end to end with a real mission run through `deliberate() -> approve() -> execute() -> PROMOTED` asserting the exact ordered history (`tests/test_council_orchestrator.py`). Still missing, and the reason this organ stays yellow rather than green: the Maintenance repair pipeline (`MaintenanceConvergenceService.run_approved_repair()`, `aios/application/maintenance/service.py`) is a separate, independently-constructed execution path with its own workspace/worker/promotion call sequence -- the same wiring pattern needs to be repeated there for this organ's "recovery and resumption" claim to hold system-wide, not just for Council missions; not attempted in this pass given the scale already covered. |
