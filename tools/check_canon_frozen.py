...
def main(argv: list[str]) -> int:
    ...
    if frozen_hits and not allow_canon:
        print(
            "TEXTURE-CANON VIOLATION — these are the operator's hand-authored texture/GLB assets"
        )
        ...
        print(
            "only his asset files are guarded. If you ARE deliberately changing a texture/asset"
        )
        print(
            "with the operator's sign-off (before/after in HIS browser), re-run with --allow-canon."
        )
        return 1
    if frozen_hits and allow_canon:
        print(
            "TEXTURE-CANON BREAK-GLASS (--allow-canon) — permitting AUTHORIZED asset edits:"
        )
        for p in frozen_hits:
            print(f"  ● {p}")
        print(
        ...
        print(
            "matters most. Change them only with his eye on before/after, and keep a backup."
        )
        return 0
    print(
        f"texture-canon OK — {len(paths)} changed path(s), no protected assets touched."
    )
    return 0
