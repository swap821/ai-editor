...
def cls_status():
    ...
    entry = next(
        (
            ...
            if x["action_type"] == "create_file"
            and x["target_shape"] == "training_ground/*.py"
        ),
        {},
    )
...
def main() -> None:
    ...
    for i in range(1, 6):
        ...
        _, entry = cls_status()
        line = (
            f"turn {i}: outcome={result['outcome']} approvals={approvals} "
        ...
        if approvals == 0 and entry.get("status") == "earned":
            print(
                f"  *** EARNED-AUTONOMY FIRED: turn {i}'s write applied with ZERO approvals."
            )
