        ...
        for k, v in data.items():
            if k.endswith("_id") and isinstance(v, str):
                if v not in allowed_refs:
                    raise ValidationError(f"Invented identifier detected: {v} is not in evidence_references")
            elif k.endswith("_ids") and isinstance(v, list):
                for item in v:
                    if isinstance(item, str) and item not in allowed_refs:
                        raise ValidationError(f"Invented identifier detected in array: {item} is not in evidence_references")
