...
class VoiceSpeakRequest(BaseModel):
    ...
    format: str = Field(
        "wav", description="'wav' for binary stream, 'json' for base64."
    )
...
async def voice_transcribe(
    ...
    if stt is None:
        raise HTTPException(
            status_code=501, detail="STT not enabled (set AIOS_VOICE_STT=true)"
        )
...
def voice_speak(
    ...
    if tts is None:
        raise HTTPException(
            status_code=501, detail="TTS not enabled (set AIOS_VOICE_TTS=true)"
        )
    from aios.core.voice import VoiceError
