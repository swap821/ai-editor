...
| **S3** | **Deployment auth + CORS hardening for the default UI.** ✅ *DONE* — `aiosAdapter.ts` reads `process.env.NEXT_PUBLIC_AIOS_TOKEN` (injected by `vite.config.js` from `VITE_AIOS_API_TOKEN`) and sends `Authorization: Bearer ${token}` when set. `AIOS_CORS_ORIGINS` is validated at startup and rejects `*` while credentials are enabled (`_validate_cors_origins` in `aios/api/main.py`, regression tests in `tests/test_cors_guard.py`). The proxy-header policy is hardened: `AIOS_TRUST_PROXY_HEADERS`/`--proxy-headers` disable the loopback token exemption and require a strong `AIOS_API_TOKEN` (`aios/__main__.py`, `aios/api/main.py`, `tests/test_token_auth_proxy_header.py`). | A token-protected/non-loopback deploy must not silently 401 the default UI or be bypassed by a proxy. | — |
