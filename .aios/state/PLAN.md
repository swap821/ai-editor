...
>
> **Test baseline, verified this session:** `pytest tests/` collects **552 tests**;
> the suite is **551 passed / 1 skipped** (the 1 skip is `test_vector_shield_catches_
> semantically_novel_injection`, gated on embedding-model availability — effectively
> 0 on a fully-provisioned host). Full estate = **551 backend + 65 product + 38 lab
> ≈ 654 green.** Grep-confirmed absences in the product path: **voice/STT/TTS
...
| **H1** | **Doc-currency sweep — pin ONE test baseline.** True count = **551 passed / 1 skipped (552 collected)**. Stale/contradictory numbers are hard-coded in ~7 docs: `AGENTS.md:124`=516, `README.md:95`=516, `START_HERE.md:54`=516, `RESUME.md` says 545/525/544 (three numbers in one file), `CEO_LOG.md:234,248`=516, `PLAN.md` body had 457, `FRONTEND_RENOVATION_BLUEPRINT.md` 516 vs 546. Pin the number in **one** place (or follow `KICKOFF_PROMPT.md:24`'s "report live counts" pattern). Also: `AGENTS.md:155` still says "~75–80%" (newer docs say ~80–85%); §XI router block predates `failover.py`/`catalog.py`/calibration; `frontend/README.md` is stock Vite boilerplate. | Docs are the single largest class of drift; `START_HERE.md`'s "expect 516" will make a healthy 551 run look anomalous. | ~3–4 hrs |
...
| **S3** | **Deployment auth + CORS hardening for the default UI.** `aiosAdapter.ts` sends **no `Authorization` header** — only the classic Vite client wires `VITE_AIOS_API_TOKEN`, so a token-protected/non-loopback deploy of the **default** superbrain UI gets 401s on every call (Phase-1b is staged-but-inert: `vite.config.js` defines the token, adapter doesn't read it). Wire the bearer; validate `AIOS_CORS_ORIGINS` rejects `*` (credentialed CORS + `allow_methods/headers=['*']` widens silently otherwise). | The default UI literally cannot authenticate to a protected backend — a real deploy blocker once the security posture tightens. | ~2–3 days |
...
| **S5** | **Test/coverage gates + cross-suite runner.** No `--cov-fail-under` anywhere; no single "run everything" command (backend pytest is silent about the 103 JS/TS tests). The product frontend is the weak leg: `App.jsx` (1875 LOC), `main.jsx`, the 10 organ ports, ForgePorts, SuperbrainShell, and the entire ported superbrain lib/scene/HUD have **zero product-side unit tests**. Add a coverage threshold, a one-shot all-suite gate, and a minimal CI workflow (`.github/` is absent). | Locks in the quality that already exists so it can't silently regress across the multi-branch activity; cheap insurance. | ~2–3 days |
