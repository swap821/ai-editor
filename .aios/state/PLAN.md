...
| **D1** | **RED-policy decision (not code)** — confirm hard-block (current, safer) vs. blueprint's typed-token confirm; write the chosen invariant into CLAUDE.md + blueprint. | P1-8 | Decision recorded; if "keep hard-block", add one test asserting `execute_approved` refuses RED (already true) and document it; if "typed-token", that becomes its own slice. |
