...
| **H1** | **Doc-currency sweep — pin ONE test baseline.** Adopt `KICKOFF_PROMPT.md:24`'s "report live counts" pattern in the living docs (`README.md`, `START_HERE.md`, `AGENTS.md`). Reconcile the `PLAN.md` S3 bearer-token claim against current code: `aiosAdapter.ts` now sends `Authorization: Bearer ${AIOS_TOKEN}` when `VITE_AIOS_API_TOKEN` is set. `frontend/README.md` is already project-specific, not stock Vite. Add superseded banners to dated snapshots (`HIDDEN_KNOWLEDGE.md`, `BACKEND_TRUE_PICTURE.md`, `CEO_LOG.md`, `FRONTEND_RENOVATION_BLUEPRINT.md`, `ARCHITECT_REVIEW_2026-06-14.md`) rather than rewriting their historical numbers. | Docs are the single largest class of drift; stale baselines make healthy runs look anomalous. | ~3–4 hrs |
...
| **S3** | **Deployment auth + CORS hardening for the default UI.** `aiosAdapter.ts` now reads `process.env.NEXT_PUBLIC_AIOS_TOKEN` (injected by `vite.config.js` from `VITE_AIOS_API_TOKEN`) and sends `Authorization: Bearer ${token}` when set. Remaining work: validate `AIOS_CORS_ORIGINS` rejects `*` (credentialed CORS + `allow_methods/headers=['*']` widens silently otherwise) and harden the proxy-header policy so a reverse proxy cannot bypass the token. | A token-protected/non-loopback deploy must not silently 401 the default UI or be bypassed by a proxy. | ~2–3 days |
