...
- **NEXT (operator's RTX, FIDELITY):** the dense brain canopy still clips white — the documented §4 follow-up is the **TSL bloom + AgX pass** (three's WebGPU `PostProcessing` + `bloom()` node, threshold 1.0) to replace the gated-off WebGL PostFX; then density/exposure via `window.__POINTFIELD_GPU` (uGlowMul/uSize live). Spike design + A/B plan: `.aios/state/GAGOS_WEBGPU_SPIKE.md`. Greenlight-to-promote is his call (lab-first law).
