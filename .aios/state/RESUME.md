...
## Single Next Action — NEXT SLICE CHOSEN: Verification-Strength Taxonomy (GAGOS roadmap Phase 1, the keystone)
Operator read `GAGOS_FINAL_ROADMAP_council.md` and chose Phase 1 as next. Goal: make "verified" mean ONE strength everywhere the learning loop trusts it. Define STRONG (sibling test suite asserting behavior) / MEDIUM (targeted assertion/typecheck) / WEAK (exit-code-only) / NONE; stamp every development/skills/swarm_patterns/mistake record + the council `TestingQueen` verdict with strength; GATE promotion (no verified skill/pattern/mistake-lesson/planner-confidence bump from below threshold; WEAK/NONE recorded but ineligible to calibrate the future); surface strength in the organism (bright vs faint imprint). Directly kills the false-green class the adversarial reviews kept catching (TestingQueen currently treats returncode==0 as uniform pass = WEAK). Brainstorm → spec → build → adversarial review (Verifier owns Phase 1 per roadmap §6). HONEST CONTEXT: roadmap argues the council layer should be THIN functions over existing agents (executor/swarm/tool_agent/self_apply) + rendered as anatomy — our build leans parallel + the dashboard violates The One Law; weigh that when shaping new work. Other roadmap priorities after: Phase 2 container-by-default execution (our worker uses host subprocess.run); Phase 3 ledger/scanner hardening.
