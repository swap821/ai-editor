...
**Last completed + verified:** Organ **18** (`MemoryAuthority`) reachability via `get_memory_authority()` — focused pytest green.

**Contract state:** 7 green / 47 yellow.

**Single next action:** Organ **19** (`EmergencyStopController`) reachability via `get_emergency_stop()`.
