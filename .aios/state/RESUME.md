...
## Single Next Action
Operator's choice — the Council now runs chat → deliberate → approve → act → report end-to-end. Tracked follow-ups (scope-bounded, not blockers): global semaphore capping concurrent worker subprocesses; bind approval to a contract/verdict hash + assert `COUNCIL_WORKSPACE_ROOT` is not a repo ancestor; per-caller mission ownership; verifier-executed write targets via the isolated backend (real-worker carry-over). Bigger slices: route the main GAGOS chat box to origination (intent + scope-picker UX); Phase 3B Queen-as-services; Phase 4 pheromones; integrate the rescued knowledge-graph/skills branches.
