...
- Added `aios/runtime/profiles.py` with `RuntimeProfile`, built-in registry loader, and persistence helpers; shipped built-in profiles (`local-first`, `operator`, `autonomous`, `air-gapped`) in `aios/runtime/data/profiles.json`.
...
- Worktree is dirty with Slice 4 changes, ready to commit and push.

**Current Slice:** Slice 5 — to be chosen by operator / next agent.

**Single Next Action:** Commit and push Slice 4 to `master`, then release the builder lease.
