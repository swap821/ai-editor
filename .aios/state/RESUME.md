...
## Single Next Action
Begin Phase 3A-lite: add durable Council state (`council_state.py`) with SQLite `queen_verdicts` and `council_events`, persist every Queen verdict/event with snapshot/mission IDs, and expose enough replay/query surface for the dashboard to stop depending only on JSON artifacts.
