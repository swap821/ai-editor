...
- **status:** DONE (2026-08-31, not in scope_lock) · **effort:** M
- **resolution:** Implemented as `aios/policy/credential_paths.py::is_credential_path`, NOT inside `scope_lock.py` — that file is frozen core (§VIII), and a denylist that must grow as new cloud providers appear does not belong behind a freeze. Wired into every non-frozen chokepoint: `tool_handlers.py` (read_file + 3 scope sites), `api/routes/files.py` (read + edit), `application/agent_bridge.py` (read AND write).
  - **The original diagnosis understated it.** A partial list already existed (`agent_bridge._SECRET_MARKERS`) at ONE call site, for reads only, and it matched substrings against the BASENAME — so every credential store identified by its *directory* was invisible. Measured before the fix: `.ssh/config`, `.aws/credentials`, `.git/config`, `.docker/config.json`, `.claude/settings.json`, `.netrc` all returned False. Writes were unguarded entirely: the bridge refused to SHOW an agent `.env` while letting it overwrite the file.
  - `looks_secret_bearing` now delegates rather than keeping a second, weaker answer.
  - `read_file` had NO scope check at all — only project-root containment plus content redaction. It is now refused by name before the file is opened; an AST test pins the ordering, because redaction is a content heuristic and `privacy_filter` shipped a live AWS-key bypass built on exactly that kind of shape reasoning.
  - **Not covered:** the shell-command path, which classifies in FROZEN `gateway.py`. Verified this is not an open hole: `cat training_ground/.env` returns RED — as does `cat` of ordinary source — so arbitrary shell reads are approval-gated at a coarser level. Wiring the denylist there would require §VIII.
  - A structural test enumerates every non-frozen module calling `is_path_in_scope` and fails if one lacks the credential guard, so the NEXT chokepoint cannot be added unguarded.
...
- **status:** BLOCKED — §VIII proposal written 2026-08-31, awaiting operator · **effort:** L
- **why blocked:** the threading crosses TWO frozen-core files. `gateway.classify()` (`aios/security/gateway.py`) and `command_stays_in_scope()` (`aios/security/scope_lock.py`) are both FROZEN; only `Executor._scope_cwd()` is editable. There is no honest non-frozen delivery.
- **severity downgraded, with evidence:** the catalog's concurrency race has NO live trigger. Measured — `grep -rn "set_scope_roots(" aios/ tools/ scripts/` excluding scope_lock itself returns **nothing**: no production path mutates the scope roots; every caller is a test. This is latent, not open.
- **real evidence it still matters:** because the state is process-global, ~9 test sites must save/restore it by hand (two written 2026-08-31 while closing other items); a forgotten restore silently redirects every later test's containment check.
- **workaround rejected on purpose:** a contextvars override in a non-frozen module would leave `gateway.classify` reading the global, giving "what is in scope?" two answers that can disagree — the exact shape behind two containment escapes already found here. A second derivation would be worse than the defect.
- **proposal:** `release/organ-2/2026-08-31-scope-context-proposal.md` (deliberately NOT in `release/section-viii/`, which means *authorized*, so a proposal there would authorize itself).
...
- **status:** DONE (2026-08-31) · **effort:** M
- **resolution:** `signature()` is now `sha256(f"{workspace_id()}|{action_type}|{norm}")`. `workspace_id()` derives from the ACTIVE scope roots — resolved per call, not at construction, so a long-lived injected ledger follows a scope change instead of its birth moment. Derived from the confinement rather than the target because `is_earned` is called with a filepath by `tool_agent` AND with a whole shell command by `PolicyKernel`; parsing a workspace out of a command string would be guesswork in the key that decides whether a human is consulted.
- **this was the LIVE path.** Both auto-grant sites — `tool_agent.py:1169` (create_file/edit_file) and `kernel.py:1014` (REQUIRE_HUMAN command) — call `is_earned`, which keys on this signature. The scoped variant `scoped_signature` (carrying `project_id`) already existed with ONE caller, `GovernedAutonomy`, which has NO production caller at all. Capability built, path never wired — the seventh instance of that mistake in this codebase.
- **migration:** `workspace_id TEXT` added to `earned_autonomy` (idempotent ALTER + CREATE TABLE). Pre-existing rows are left NULL and are UNREACHABLE by construction, since their key lacked a workspace component. Deliberate and safe-direction: the class must be re-earned, whereas back-filling a workspace we cannot know would grant autonomy nobody verified there. Proven against a real pre-migration DB carrying an `earned` row with a streak of 9 — it no longer grants.
- **caveat:** `workspace_id()` reads the process-global scope roots, so it is exactly right today (one active set per process) and needs item 3's `ScopeContext` before concurrent lanes.
...
- **status:** PARTIAL (2026-08-31 — CI diff-gate DONE; relocation NOT done) · **effort:** M
- **resolution (CI diff-gate only):** `scripts/check_frozen_core.py`, wired into ci.yml. Until it existed the §VIII rule had exactly two enforcers (`ConstitutionEnforcer.check_file_edit`, `SelfAnalysisAgent.classify_target`), both in-process — so a commit authored any other way met no automated resistance at the merge boundary. The gate reads `FROZEN_PATH_PREFIXES` from `aios/policy/constitution.py`, the SAME constant the runtime enforcer uses, so gate and product cannot disagree; a test asserts identity, not equality of two copies.
  - **Honest about what it is:** a tripwire that forces ceremony, not an authorization system. The real authority is the human who merges. A frozen-path diff fails unless a `release/section-viii/` record naming the exact path is added IN THE SAME DIFF — a record from an earlier commit does not authorize, or one approved change would become a standing permission. Proven end-to-end against real git history in a temp repo, including the stale-record and partial-record cases.
  - **CLOSED 2026-09-01 — the constants moved; `http_guard.py` will NOT be done (operator decision).** `SCOPE_ROOTS` / `EARNED_AUTONOMY_*` / `ROUTER_CLOUD_TASKS` / `MAX_RED_ACTIONS_PER_SESSION` / `AUDIT_GENESIS_HASH` now live in `aios/security/limits.py` (#272), and `classify_target` derives its frozen set from `FROZEN_PATH_PREFIXES` rather than its own `frozen_subdirs` default — which is what "widen frozen_subdirs" actually needed, since the two lists agreed only by coincidence of content.
  - **Do not re-propose the `http_guard.py` relocation.** The catalogue's premise is stale: the CORS/Bearer logic already left `aios/api/main.py`. `_validate_cors_origins` is a one-line delegation to `aios/interfaces/http/edge_security.py`, a cohesive module with a real authority class. Moving ~400 lines of working security code to change its directory is churn with real regression risk, and any protection gain is also available by widening the frozen prefix. Ruled out by the operator on 2026-09-01 after the trade-off was put to them; this is a decision, not an oversight. Deliberately deferred: it is a wide-blast-radius refactor of a module imported everywhere, and it permanently moves live config INSIDE the frozen prefix, so every later tuning of those constants would need §VIII. That is an architectural call for the operator, not a mechanical follow-on.
...
### 5b. Importing `aios` materializes the SQLite stores as an import side effect
- **status:** FIXED (2026-08-31) · **effort:** M
- **root cause, traced not guessed:** instrumented `sqlite3.connect` during import. `aios/api/deps.py` built FOUR DB-backed singletons in its module body — `_CAPABILITIES`, `_RATE_LIMITER`, `_SESSION_MANAGER`, `_PRIVACY_AUDIT_TRACKER` — and each constructor initialises its schema, which CREATES the file. The chain was `aios/policy/__init__.py` → `interfaces/http/edge_security.py:21` → `from aios.api.deps import get_session_manager` → four databases. A fifth lived in `api/routes/system.py` (`_EPISODIC = get_memory_authority()...`).
- **fix:** all five construct on FIRST USE via `_lazy_singleton` (double-checked under a lock — the pattern `get_identity_service` in the same file already used). Module `__getattr__` (PEP 562) keeps `deps._CAPABILITIES` working as an attribute, because four test modules reach for it directly to install fakes; a rewrite that turned those into `None` would have broken that surface for no reason. Internal call sites use the `_capabilities()` accessors, since a bare global lookup does NOT consult module `__getattr__`.
- **measured before → after** (empty `AIOS_DATA_DIR`): `import aios.policy` 4 → **0**; `import aios.api.deps` 4 → **0**; `import aios.api.routes.system` 1 → **0**; `scripts/check_frozen_core.py` 4 → **0**.
- **still provisions, and correctly:** `import aios.api.main` (7) — building the FastAPI app at module level IS what that import means. `tools/thesis_audit.py` (7) because it dynamically imports evidence modules including the app. The line drawn is between "I imported a library" and "I started the app"; the ci.yml `AIOS_DATA_DIR` redirect is therefore KEPT for thesis_audit.
- **regression guard:** `tests/test_import_creates_no_databases.py` runs each import in a SUBPROCESS with its own data dir — an in-process test would pass merely because the session had already imported everything.
- **what:** `import aios` CREATES `aios_identity.db`, `aios_constitution_snapshots.db`, `aios_approvals.db`, `aios_capabilities.db`, `aios_sessions.db`, `aios_privacy_audits.db` and `aios_memory.db` under `AIOS_DATA_DIR` — before any caller asks for anything. Discovered 2026-08-31 the hard way: adding `tools/thesis_audit.py` and `scripts/check_frozen_core.py` (both pure static analysis) to the release-authority gates step created those files as the RUNNER uid, and the executor topology later in the same job runs as a DIFFERENT uid, so three blocking gates went PARTIAL with `OperationalError: attempt to write a readonly database` — `operator_identity`, `mutation_authority`, `emergency_stop_controller`. The pre-existing gate scripts create only directories, which is why it had never bitten.
- **worked around by:** redirecting `AIOS_DATA_DIR` to a throwaway dir for those two commands in ci.yml, and making `check_frozen_core.py` self-isolating so it is safe regardless of caller. The underlying side effect is untouched.
- **risk if skipped:** Every future tool, gate, lint or one-off script that imports `aios` silently provisions databases wherever it happens to run, with whatever ownership the invoking user has. The failure surfaces far from the cause and reads like a permissions bug in the product rather than an import side effect.
...
- **status:** not-started · **effort:** L
- **what:** aios/agents/self_analysis_agent.py classify_target()/SelfAnalysisAgent.__init__ still default `frozen_subdirs: tuple[str, ...] = ("security",)` (lines 132, 189, 212) — verified unchanged. AUDIT_GENESIS_HASH, SCOPE_ROOTS, EARNED_AUTONOMY_*, ROUTER_CLOUD_TASKS, MAX_RED_ACTIONS_PER_SESSION all still live in aios/config.py; `find aios/security` shows no limits.py or http_guard.py. **[SUPERSEDED 2026-09-01: limits.py now exists (#272); http_guard.py was ruled out by the operator — see item 5.]** `.github/workflows/ci.yml` has no job matching frozen/SCOPE_ROOTS/AUDIT_GENESIS_HASH (grep returned zero matches) — no automated CI backstop blocks a PR that touches a frozen path.
...
- **status:** not-started · **effort:** S
- **what:** Grep of .github/workflows/*.yml confirms no job checks whether a PR's diff touches a frozen path. Add a CI job to ci.yml that fails any PR whose diff touches aios/security/**, the audit genesis hash, or (once relocated per 0.3) aios/security/limits.py **[SUPERSEDED 2026-09-01: the CI diff-gate shipped as scripts/check_frozen_core.py (#269/#272); http_guard.py ruled out — see item 5]** — an automated backstop independent of self_analysis_agent.py's classify_target()/frozen_subdirs runtime check, which today only covers aios/security/ and is enforced in-process, not at the CI-merge boundary.
...
- **status:** DONE (2026-08-31) · **effort:** S
- **resolution:** Both scripts are now collectible tests, and the diagnosis in the original entry was incomplete. Neither file contained a single `def test_` — both were `main() -> int` scripts — so the proposed rename would have collected zero tests. Both also failed standalone with `403 Mutation requires a bearer token or a valid session, exact Origin, and session-bound CSRF proof`: a guard added after their last edit (2026-06-25). `tests/conftest.py` seeds that proof into every loopback TestClient, so running under pytest is what repairs it.
  - `e2e_yellow_verify.py` → `tests/e2e/test_yellow_approval_to_verify_pass.py`. The flow is still live; the scenario is preserved and passes against a real `qwen2.5-coder:7b`. Marked `e2e_live` and skipped unless `AIOS_E2E_LIVE=1`, because a default-CI test that needs an external daemon turns the gate red for reasons unrelated to the change under review. It also now saves and restores `scope_lock.set_scope_roots` / `config.SCOPE_ROOTS`, which are process-wide; the script could leak them because it owned its own process, a test cannot.
  - **Not fully closed:** the live test's verify step (4-5 of 5) has not been observed passing. This laptop has no Docker daemon, so verification fails closed — `[VERIFY FAIL] 0 passed, 0 failed (exit 1) (strength=NONE)` / `failed to connect to the docker API at npipe:////./pipe/docker_engine` — which is the designed behaviour, not a regression. Steps 1-3 WERE observed on a real `qwen2.5-coder:7b`, including that the file is byte-identical on disk when `human_required` is emitted. A container precondition now skips the test rather than weakening the assertion; the end-to-end green still needs a host with both daemons up. Related: item 84 (no CI job ever builds or runs the executor image).
  - `e2e_cloud_burst.py` → `tests/e2e/test_swarm_strategy_is_gated.py`. The demonstrated path is gated off ON PURPOSE: `generate_pipeline` short-circuits `req.swarm or req.role_pass` to `error: strategy_unavailable` (c8e3da0e, 2026-07-15 — three weeks after the script was last touched), so a `cloud_route` frame can no longer be emitted and renaming the script would have wired in a permanently red test. Grep found NO test anywhere covering `strategy_unavailable`, so the fail-closed gate that retired the whole swarm path had no regression guard at all. That gap — not the filename — was the real finding, and the replacement test closes it.
...
- **status:** MOSTLY DONE (2026-09-01) · **effort:** M
- **resolution:** `.github/workflows/nightly.yml` — `schedule: cron 17 3 * * *` plus `workflow_dispatch` (a scheduled workflow with no manual trigger cannot be debugged without waiting a day). Two jobs, each with a real Ollama daemon and a live backend:
  - **learning-loop prover** — the claim item 88 named. Runs `--lenient`, which is the prover's OWN supported way to downgrade LLM-obedience-dependent checks to warnings while structural checks still fail. So the exit code is a real gate on the learning CHAIN, deliberately NOT `|| true`. Uploads `.aios/audit/learning-loop-runs.jsonl` as an artifact, which is the half item 88 actually complained about: the evidence existed only on one laptop.
  - **endurance** — a 10-minute pass, gated by `scripts/ci_nightly_endurance_check.py` on HARNESS INTEGRITY, never score, following `ci_local_cohort_check.py`'s rule and for its documented reason (nine infrastructure defects in 2026-08-18 each looked exactly like a low model score). RED from a 0.5b model on a 2-core runner is expected and passes; a run that never reached its summary, attempted zero turns, aborted on consecutive errors, or shows a traceback/auth/DB failure does not.
- **both jobs wait on `/ready`, not `/health`** — the liveness probe returns ok from a process whose DB is unwritable and which can reach no model, so waiting on it would start a run against a broken backend and blame the model. `/ready` was added in #269 for exactly this.
- **STILL DISPATCH-ONLY:** `golden-cohort-local` and `release-strict-gate` in ci.yml (`if: github.event_name == 'workflow_dispatch'`). `golden_mission_runner.py` IS invoked by a job — so item 88's 'zero tools/ references' is stale — but only when someone remembers to dispatch it. Scheduling them means either adding a `schedule:` trigger to ci.yml (which would run EVERY job nightly, duplicating each PR check) or a third copy of the Ollama+backend setup in nightly.yml. Left undone deliberately rather than smuggled in; it is a trigger-model decision, not a mechanical follow-on.
