...
| R15-03 | Promotion capability token/digest guessing | `aios/application/promotion/authority.py`, `aios/api/deps.py`, `aios/application/maintenance/service.py` | INTEGRATION | PASS | PASS (`PromotionAuthorization` wired; maintenance never synthesizes `ConsumedCapabilityProof` — the route passes the guard/authority-produced proof, absence refuses `capability_binding_missing`) | PASS | PASS | PENDING | N/A | N/A | ABSENT | REPAIRED (integration level; note: `PromotionAuthority` does not yet verify authority *ownership* of the proof object — a shape-valid forged proof is not cryptographically distinguished; follow-up required) |
