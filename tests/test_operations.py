...
from aios.operations.tracing import bind_trace_context, get_trace_context, new_trace_context
...
def test_doctor_reports_executor_as_fatal_only_in_production(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    demo = doctor_report(profile="demo", project_roots=(tmp_path,), executor_probe=lambda: (False, "not available"))
    production = doctor_report(profile="production", project_roots=(tmp_path,), executor_probe=lambda: (False, "not available"))

    assert demo.ok
    assert any(check.status == "warning" and check.name == "executor" for check in demo.checks)
    assert not production.ok
    assert {check.name for check in production.checks if check.status == "fatal"} >= {"executor", "operator_token"}


def test_backup_manifest_excludes_environment_files_and_round_trips_state(tmp_path: Path) -> None:
    data = tmp_path / "data"
    data.mkdir()
...
def test_projection_rebuild_replays_only_durable_observations(tmp_path: Path) -> None:
    ...
    bus.append("worker.started", "worker-1", {"worker_id": "worker-1", "role": "tester"})
