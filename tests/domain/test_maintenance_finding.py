...
def test_engine_reopens_suppressed_finding_on_reappearance(base_finding):
    ...
    new_report = base_finding.model_copy(
        update={"last_seen": "2026-07-18T01:00:00Z", "target_digest": "hash3"}
    )
...
def test_engine_blocks_unauthorized_resolution(base_finding):
    ...
    with pytest.raises(SecurityViolationError, match="not authorized"):
        engine.attempt_resolution(
            base_finding, actor="local_model", deterministic_evidence="foo"
        )

    with pytest.raises(SecurityViolationError, match="not authorized"):
        engine.attempt_resolution(
            base_finding, actor="cloud_model", deterministic_evidence="foo"
        )
...
def test_engine_rejects_free_form_resolution_without_structured_evidence(base_finding):
    ...
    with pytest.raises(
        SecurityViolationError, match="requires complete governed maintenance evidence"
    ):
        engine.attempt_resolution(
            base_finding, actor="system_verifier", deterministic_evidence=None
        )
...
def test_engine_rejects_free_form_resolution_even_with_text(base_finding):
    ...
    with pytest.raises(
        ...
        engine.attempt_resolution(
            ...
            deterministic_evidence="Scan clean: 0 unused code found.",
        )
