...
class TestSingleFailureRevocation:
    ...
    def test_failure_revokes_probation(self, enabled_ledger, monkeypatch):
        ...
        result = enabled_ledger.record_outcome(
            "create", "training_ground/test.py", success=False
        )
    ...
    def test_failure_revokes_earned(self, enabled_ledger, monkeypatch):
        ...
        # Build up to earned
        for _ in range(3):
            enabled_ledger.record_outcome(
                "create", "training_ground/test.py", success=True
            )
        # Should be earned
    ...
    def test_failure_first_outcome_revokes(self, enabled_ledger, monkeypatch):
        ...
        result = enabled_ledger.record_outcome(
            "edit", "training_ground/new.py", success=False
        )
        assert result["status"] == "revoked"
    ...
    def test_failure_does_not_decrement_success_count(
        self, enabled_ledger, monkeypatch
    ):
...
class TestSuccessStreakCounting:
    ...
    def test_weak_success_cannot_build_autonomy_streak(
        self, enabled_ledger, monkeypatch
    ):
    ...
    def test_weak_success_revokes_already_earned_autonomy(
        self, enabled_ledger, monkeypatch
    ):
...
class TestSignatureNormalization:
    ...
    def test_same_shape_same_signature(self, enabled_ledger, monkeypatch):
        ...
        assert sig1 == sig2, (
            "Same dir + same extension should normalize to same signature"
        )
    ...
    def test_signature_secrets_redacted(self, enabled_ledger, monkeypatch):
        ...
        sig_with_secret = enabled_ledger.signature("create", "training_ground/test.py")
        sig_clean = enabled_ledger.signature("create", "training_ground/test.py")
...
class TestLedgerObservability:
    ...
    def test_earned_count_accurate(self, enabled_ledger, monkeypatch):
        ...
        for _ in range(3):
            enabled_ledger.record_outcome(
                "create", "training_ground/a.py", success=True
            )
        for _ in range(3):
            enabled_ledger.record_outcome(
                "create", "training_ground/b.txt", success=True
            )
