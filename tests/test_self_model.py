...
def test_strength_from_high_rate_over_min_attempts() -> None:
    model = synthesize_self_model(
        _FakeDev({"coding": (10, 0.9)}), _FakeMistakes([]), min_attempts=4
    )
...
def test_soft_spot_from_low_rate() -> None:
    model = synthesize_self_model(
        _FakeDev({"reasoning": (6, 0.33)}), _FakeMistakes([]), min_attempts=4
    )
...
def test_no_trait_below_min_attempts() -> None:
    ...
    model = synthesize_self_model(
        _FakeDev({"coding": (2, 1.0)}), _FakeMistakes([]), min_attempts=4
    )
...
def test_caution_from_recurring_verified_lesson() -> None:
    model = synthesize_self_model(
        ...
        _FakeMistakes(
            [
                {
                    "lesson_text": "re-run the failing test before claiming done",
                    "occurrence_count": 3,
                }
            ]
        ),
...
def test_render_is_first_person_and_cites_evidence() -> None:
    model = synthesize_self_model(
        _FakeDev({"coding": (9, 0.9)}), _FakeMistakes([]), min_attempts=4
    )
    text = render(model)
...
def test_render_empty_model_is_empty_string() -> None:
    assert (
        render(synthesize_self_model(_FakeDev({}), _FakeMistakes([]), min_attempts=4))
        == ""
    )
...
def test_task_profile_counts_only_verified(tmp_path: Path) -> None:
    ...
    dev.record(
        "do the coding task", "unverified", metadata={"task": "coding"}
    )  # excluded
