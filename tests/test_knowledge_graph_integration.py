...
class TestSkillIngestion:
    def test_ingestion_hook_fires_on_skill_promotion(
        self, db: Path, facts: SemanticFacts
    ) -> None:
    ...
    def test_candidate_skill_produces_no_edges(
        self, db: Path, facts: SemanticFacts
    ) -> None:
        skills = SkillMemory(db, min_successes=3, min_success_rate=0.8, facts=facts)
...
class TestMistakeIngestion:
    def test_ingestion_hook_fires_on_mistake_promotion(
        self, db: Path, facts: SemanticFacts
    ) -> None:
        ...
        mid = mistakes.record(
            "task-1",
            "TypeError",
            "missing null check in 'parser.py'",
            "added guard clause",
    ...
    def test_pending_mistake_produces_no_edges(
        self, db: Path, facts: SemanticFacts
    ) -> None:
        mistakes = MistakeMemory(db, facts=facts)
        mistakes.record(
...
class TestOutcomeIngestion:
    def test_ingestion_hook_fires_on_outcome_record(
        self, db: Path, facts: SemanticFacts
    ) -> None:
        tracker = DevelopmentTracker(db, facts=facts)
        tracker.record(
            "Deploy 'auth-service.py' to staging", "verified_success", tool_calls=5
        )
        rows = _fact_rows(facts)
        assert len(rows) > 0, "verified_success should ingest edges"
        assert any(r["predicate"] == "has_verified_success" for r in rows)

    def test_unverified_outcome_produces_no_edges(
        self, db: Path, facts: SemanticFacts
    ) -> None:
        tracker = DevelopmentTracker(db, facts=facts)
...
class TestFailSoft:
    def test_skill_ingestion_is_fail_soft(self, db: Path, facts: SemanticFacts) -> None:
        ...
        assert isinstance(skill_id, int), (
            "record_attempt must succeed despite ingestion failure"
        )

    def test_mistake_ingestion_is_fail_soft(
        self, db: Path, facts: SemanticFacts
    ) -> None:
        mistakes = MistakeMemory(db, facts=facts)
        mid = mistakes.record(
        ...
        assert row["verification_status"] == "verified", (
            "promote must succeed despite ingestion failure"
        )

    def test_outcome_ingestion_is_fail_soft(
        self, db: Path, facts: SemanticFacts
    ) -> None:
        tracker = DevelopmentTracker(db, facts=facts)
        with patch.object(facts, "add_fact", side_effect=RuntimeError("boom")):
            row_id = tracker.record(
                "Fix 'api.py' endpoint", "verified_success", tool_calls=2
            )
