...
class TestHelpers:
    def test_infer_skill_data_structures(self):
        ...
        assert (
            _infer_skill("implement a stack with push and pop")
    ...
    def test_extract_module_name(self):
        ...
        assert (
            _extract_module_name("Create training_ground/my_mod.py with stuff")
            == "my_mod"
        )
...
class TestMineFromDevelopment:
    ...
    def test_deduplicates_against_existing(self, seed_db):
        ...
        miner = CurriculumMiner(db_path=seed_db)
        first_batch = miner.mine_from_development(
            existing_prompts=set(), max_proposals=5
        )
        existing = {p.prompt for p in first_batch}
        second_batch = miner.mine_from_development(
            existing_prompts=existing, max_proposals=5
        )
