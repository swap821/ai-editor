...
def test_agent_recovers_allowlisted_tool_call_emitted_as_json_text() -> None:
    ...
    assert any(
        e.get("tool") == "read_file" and e["type"] == "tool_call" for e in events
    )
    assert any(
        e.get("tool") == "read_file" and e["type"] == "tool_result" for e in events
    )
...
def test_agent_never_executes_python_expression_in_text_tool_fallback() -> None:
    ...
    events = list(
        ToolAgent(chat, _executor(), max_iters=2).run(
...
def test_agent_retries_once_when_user_explicitly_requests_a_tool() -> None:
    ...
    events = list(
        ToolAgent(chat, _executor(), max_iters=4).run(
            [
                {
                    "role": "user",
                    "content": "Use the read_file tool to inspect greeter.py",
                }
            ]
        )
    )
...
def test_agent_surfaces_llm_error() -> None:
    ...
    events = list(
        ToolAgent(Boom(), _executor(), max_iters=3).run(
...
def test_agent_reflects_on_command_failure() -> None:
    ...
    def hook(command: str, error_output: str):
        ...
        return {
            ...
            "lesson_text": "add a guard clause",
            "recurrence": False,
            "mistake_id": 7,
        }
...
def test_agent_promotes_lesson_after_corrective_success() -> None:
    ...
    assert confirmed == [7], (
        "the lesson should be promoted after the corrective success"
    )
    verify_steps = [
        e
        for e in events
        if e.get("tool") == "reflect" and "Verified" in e.get("output", "")
    ]
...
def test_agent_does_not_verify_failed_command_lesson_on_unrelated_success() -> None:
    ...
    assert confirmed == [], (
        "unrelated success must not verify the failed-command lesson"
    )
...
def test_agent_does_not_reflect_on_security_block() -> None:
    ...
    events = list(
        ToolAgent(chat, _executor(), max_iters=3, on_failure=hook).run(
            [{"role": "user", "content": "delete everything"}]
        )
    )
...
def test_agent_stops_at_step_cap() -> None:
    ...
    looping = ScriptedChat(
        [_tool_call("read_directory", {"path": "."}) for _ in range(10)]
    )
    events = list(
        ToolAgent(looping, _executor(), max_iters=2).run(
...
def test_agent_edit_resolves_project_relative_sandbox_path(
    ...
    try:
        chat = ScriptedChat(
            [
                _tool_call(
                    "edit_file",
                    {
                        "filepath": "training_ground/data.json",
                        "old_string": "hello",
                        "new_string": "hi",
                    },
                ),
        ...
        assert "human_required" in types, (
            "a project-relative sandbox path must resolve, not 404"
        )
        hr = next(e for e in events if e["type"] == "human_required")
        ...
        assert (sand / "data.json").read_text(
            encoding="utf-8"
        ) == '{ "greeting": "hello" }\n'
    finally:
        scope_lock.set_scope_roots(list(original))
...
def test_agent_edit_applies_when_approved_and_snapshots(sandbox) -> None:
    ...
    agent = ToolAgent(
        ...
        max_iters=3,
        approved_edits=[
            {"filepath": "greeting.txt", "old_string": "world", "new_string": "there"}
        ],
...
def test_agent_edit_audits_applied_write(sandbox) -> None:
    ...
    chat = ScriptedChat(
        [
            _tool_call(
                "edit_file",
                {"filepath": "conf.txt", "old_string": "1", "new_string": "2"},
            ),
            {"role": "assistant", "content": "done"},
        ]
    )
...
def test_agent_edit_rejects_nonunique_old_string(sandbox) -> None:
    ...
    chat = ScriptedChat(
        [
            _tool_call(
                "edit_file",
                {"filepath": "dup.txt", "old_string": "a", "new_string": "b"},
            ),
...
def test_agent_edit_applies_approved_despite_model_arg_drift(sandbox) -> None:
    ...
    agent = ToolAgent(
        ...
        snapshot=lambda msg="": None,
        audit_log=lambda *a, **k: None,
    )
    list(agent.run([{"role": "user", "content": "edit"}]))
    assert (
        f.read_text(encoding="utf-8") == "hello there\n"
...
def test_agent_edit_atomic_publish_failure_preserves_original(
    sandbox, monkeypatch
) -> None:
    f = sandbox / "greeting.txt"
    f.write_text("hello world\n", encoding="utf-8")
...
def test_agent_create_pauses_with_preview(sandbox) -> None:
    ...
    chat = ScriptedChat(
        [
            _tool_call(
                "create_file", {"filepath": "new.py", "content": "print('hi')\n"}
            ),
            {"role": "assistant", "content": "should not be reached"},
        ]
    )
...
def test_agent_create_applies_approved_and_snapshots_despite_drift(sandbox) -> None:
    ...
    chat = ScriptedChat(
        [
            _tool_call(
                "create_file", {"filepath": "sub/new.py", "content": "WRONG DRIFT\n"}
            ),
            {"role": "assistant", "content": "Done."},
        ]
    )
...
def test_agent_create_refuses_existing_file(sandbox) -> None:
    ...
    chat = ScriptedChat(
        [
            _tool_call(
                "create_file", {"filepath": "exists.txt", "content": "clobbered\n"}
            ),
            {"role": "assistant", "content": "blocked"},
        ]
    )
...
def test_agent_pre_applies_granted_creation_when_model_ignores_it(sandbox) -> None:
    ...
    pre_applied = [
        ...
        and str(e.get("id", "")).startswith("grant-create")
    ]
...
def test_agent_pre_applies_all_grants_before_verifying(sandbox) -> None:
    ...
    agent = ToolAgent(
        ...
        approved_creations=[
            {
                "filepath": "test_thing.py",
                "content": "from thing import f\n\ndef test_f():\n    assert f() == 1\n",
            },
    ...
    verifies = [
        i for i, e in enumerate(events) if str(e.get("id", "")).startswith("autoverify")
    ]
...
def test_agent_pre_applies_granted_edit_and_skips_when_landed(sandbox) -> None:
    ...
    grant = [
        {"filepath": "mod.py", "old_string": "value = 1", "new_string": "value = 2"}
    ]
    chat = ScriptedChat(
        [
...
def test_agent_create_blocked_out_of_scope(tmp_path, monkeypatch) -> None:
    ...
    try:
        chat = ScriptedChat(
            [
                _tool_call(
                    "create_file", {"filepath": "aios/x.py", "content": "x = 1\n"}
                ),
                {"role": "assistant", "content": "blocked"},
            ]
        )
...
def test_agent_create_blocked_path_escape(sandbox) -> None:
    chat = ScriptedChat(
        [
            _tool_call(
                "create_file", {"filepath": "../../../../etc/evil", "content": "x"}
            ),
            {"role": "assistant", "content": "blocked"},
        ]
    )
...
def test_agent_create_atomic_publish_failure_leaves_target_absent(
    sandbox, monkeypatch
) -> None:
...
def test_agent_verify_reports_pass_and_does_not_reflect() -> None:
    ...
    results = [
        e for e in events if e["type"] == "tool_result" and e.get("tool") == "verify"
    ]
...
def test_agent_verify_requires_human_approval() -> None:
    ...
    events = list(
        ToolAgent(chat, _passing_executor(), max_iters=2).run(
...
def test_agent_verify_reports_fail_and_reflects_once() -> None:
    ...
    def hook(command: str, error_output: str):
        ...
        return {
            "error_type": "AssertionError",
            "lesson_text": "fix the broken case",
            "recurrence": False,
            "mistake_id": 11,
        }
    ...
    events = list(
        ToolAgent(
            ...
            approved_commands=["pytest"],
            on_failure=hook,
        ).run([{"role": "user", "content": "verify the change"}])
    )
...
def test_agent_verify_red_command_is_refused_by_gateway_not_run() -> None:
    ...
    chat = ScriptedChat(
        [
            ...
            {
                "role": "assistant",
                "content": "Could not verify — that command was blocked.",
            },
        ]
    )
    reflected: list[str] = []
    events = list(
        ToolAgent(
            chat, ex, max_iters=3, on_failure=lambda c, o: reflected.append(c)
        ).run([{"role": "user", "content": "verify by wiping the disk"}])
    )

    blocked = [
        e for e in events if e["type"] == "tool_blocked" and e.get("tool") == "verify"
    ]
...
def test_agent_verify_success_confirms_pending_lesson() -> None:
    ...
    assert confirm_events, (
        "a verify-tool success on the exact failed command must confirm it"
    )
    assert confirmed == [42]
    assert events[-1]["type"] == "done"
...
def test_agent_confirms_recalled_pending_lesson_across_run_boundary() -> None:
    ...
    assert confirm_events, (
        "a seeded recalled lesson must confirm when its exact command succeeds"
    )
    assert confirmed == [99]
...
def test_agent_recalled_pending_not_confirmed_by_unrelated_command() -> None:
    ...
    assert confirmed == [], (
        "an unrelated command's success must not confirm a recalled lesson"
    )
...
def test_agent_verify_success_does_not_confirm_a_different_command() -> None:
    ...
    assert not confirm_events, (
        "a success on a DIFFERENT command must not confirm the pending lesson"
    )
    assert confirmed == []
    assert events[-1]["type"] == "done"
...
def test_agent_plan_lists_ordered_steps() -> None:
    ...
    events = list(
        ...
        ).run([{"role": "user", "content": "refactor the parser safely"}])
    )

    results = [
        e for e in events if e["type"] == "tool_result" and e.get("tool") == "plan"
    ]
    ...
    assert planner_llm.calls, (
        "the planner's COMPLETION client (.complete) must be called"
    )
    assert events[-1]["type"] == "done"
...
def test_agent_plan_flags_low_confidence_step_for_human_review() -> None:
    ...
    events = list(
        ToolAgent(
            ...
            planner_llm=planner_llm,
            mistakes=EmptyPlannerMemory(),
        ).run([{"role": "user", "content": "do the risky thing"}])
    )
...
def test_agent_plan_surfaces_planner_error_cleanly() -> None:
    ...
    assert (
        results and results[0]["type"] == "tool_result"
    )  # surfaced cleanly, not blocked
...
def test_agent_auto_verifies_after_edit_and_reports_pass(sandbox, monkeypatch) -> None:
    ...
    verify = [
        e for e in events if e.get("tool") == "verify" and e["type"] == "tool_result"
    ]
    assert verify, "an approved .py edit must AUTO-run its sibling test"
    assert "VERIFY PASS" in verify[0]["output"]
    assert verify[0]["id"].startswith("autoverify-"), (
        "the verify was forced by the loop, not the model"
    )
...
def test_agent_auto_verify_fail_overrides_false_success_and_reflects_once(
    sandbox, monkeypatch
) -> None:
    ...
    chat = ScriptedChat(
        [
            ...
            {
                "role": "assistant",
                "content": "Done — the bug is fixed and it passes now.",
            },
        ]
    )
    reflected: list[tuple[str, str]] = []
    ...
    def hook(command: str, error_output: str):
        ...
        return {
            "error_type": "AssertionError",
            "lesson_text": "greet still drops the name",
            "recurrence": False,
            "mistake_id": 5,
        }
    ...
    assert verify and "VERIFY FAIL" in verify[0]["output"], (
        "the forced verify must contradict the model's prose"
    )
    assert len(reflected) == 1, "a genuine verify FAIL must reflect exactly once"
    assert "pytest" in reflected[0][0]
    assert not any(e.get("tool") == "reflect" for e in events), (
        "no double-reflect from the write path"
    )


def test_agent_auto_verify_reports_unverified_when_no_test(
    sandbox, monkeypatch
) -> None:
    ...
    assert (
        verify
        and "UNVERIFIED" in verify[0]["output"]
        and "SKIPPED" in verify[0]["output"]
    )
...
def test_agent_no_auto_verify_for_non_python_write(sandbox, monkeypatch) -> None:
    ...
    assert not any(e.get("tool") == "verify" for e in events), (
        "text edits aren't auto-verified"
    )
...
def test_browse_pauses_for_approval_without_pre_approval() -> None:
    ...
    assert not any(
        e.get("tool") == "browse" and e["type"] == "tool_result" for e in events
    )
...
def test_browse_blocks_local_and_private_urls() -> None:
    ...
    events = list(
        ToolAgent(
            ...
            _executor(),
            max_iters=3,
            approved_commands=["browse http://localhost:8000"],
...
def test_browse_fetches_and_extracts_text(monkeypatch) -> None:
    ...
    events = list(
        ToolAgent(
            ...
            _executor(),
            max_iters=3,
            approved_commands=["browse https://example.com"],
        ).run([{"role": "user", "content": "read example.com"}])
    )
    result = [
        e for e in events if e.get("tool") == "browse" and e["type"] == "tool_result"
    ]
