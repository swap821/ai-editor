...
- **Run with observability stack:** `AIOS_API_TOKEN=<32-char-token> docker compose up --build` starts the API + Prometheus + Grafana + Alertmanager. Grafana is on http://localhost:3000 (default admin password `admin` unless overridden with `AIOS_GRAFANA_ADMIN_PASSWORD`). Prometheus is on http://localhost:9090. Alertmanager is on http://localhost:9093. Edit `observability/alertmanager.yml` (or bind a custom file) to route alerts to a real webhook; the default sink is a dummy so the stack starts cleanly on a single-operator laptop.
...
- **Organ ledger Decisions A & B (2026-07-27):** `authority_owner` in `.aios/state/ORGAN_GREEN_LEDGER.json` is a **class reference**, not a label — `validate_ledger()` (`aios/application/governance/organ_ledger.py`) now requires it be defined as a real class inside that organ's own `production_entrypoints`. This demoted 33 of 38 then-green organs to yellow in one commit (5 survivors: organs 9, 15, 16, 18, 19). Live evidence (`requires_live_evidence`) gates green **per organ**, set `true` only where achievable in this environment, `false` only with a named non-fabricatable blocker. Full reasoning, the regression list, and current counts (5 green / 49 yellow): `docs/architecture/GAGOS_54_ORGANS.md`. **Note for whoever attempts organs 1-5** (Security Gateway/Scope Lock/Secret Scanner/Audit Logger/Injection Shield): their `production_entrypoints` are exactly the frozen security spine named in §VIII above — building a real owner class there is RED/frozen-core work, not an ordinary Phase-2 wiring task.
